...
void app_core_init(void) {
  ...
  // (Esto debe arrancar SIEMPRE, ya sea para grabar en producción o leer en
  // simulador)
  logger_init();
  ...
  xTaskCreatePinnedToCore(telemetria_ui_task, "UI_TASK", 4096, NULL, 5, NULL,
                          0);
...
// =========================================================
  ...
  xTaskCreatePinnedToCore(imu_sampler_task, "IMU_TASK", 4096, NULL, 20, NULL,
                          1);
  xTaskCreatePinnedToCore(mic_sampler_task, "MIC_TASK", 4096, NULL, 21, NULL,
                          1);
#else
  ...
  ESP_LOGW(TAG, ">>> RAM Libre actual: %ld bytes <<<",
...
void app_core_iniciar_grabacion_parametros(const char *actividad,
                                           int demora_segundos) {
...
void app_core_get_device_name(char *out_name, size_t max_len) {
  ...
  if (nvs_open("storage", NVS_READONLY, &my_handle) == ESP_OK) {
    ...
    if (nvs_get_str(my_handle, "dev_name", out_name, &required_size) ==
        ESP_OK) {
