...
#include "lvgl.h"
// #include "lv_demos.h" <-- ELIMINADO para no usar memoria extra
...
// ============================================================================
// --- LÓGICA DE LA INTERFAZ DEL LOGGER ---
// ============================================================================
static void boton_texto_event_cb(lv_event_t * e) {
    lv_event_code_t code = lv_event_get_code(e);
    lv_obj_t * btn = lv_event_get_target(e);
    lv_obj_t * label = lv_obj_get_child(btn, 0);

    if(code == LV_EVENT_CLICKED) {
        const char * texto_actual = lv_label_get_text(label);

        if(strcmp(texto_actual, "REPOSO") == 0) {
            lv_label_set_text(label, "GRABANDO");
            lv_obj_set_style_bg_color(btn, lv_palette_main(LV_PALETTE_RED), 0);
        } else {
            lv_label_set_text(label, "REPOSO");
            lv_obj_set_style_bg_color(btn, lv_palette_main(LV_PALETTE_BLUE), 0);
        }
    }
}

static void ui_init_logger_screen(void) {
    // Fondo negro
    ...
    lv_obj_add_event_cb(btn1, boton_texto_event_cb, LV_EVENT_ALL, NULL);

    // Etiqueta
    lv_obj_t * label = lv_label_create(btn1);
    lv_label_set_text(label, "REPOSO");
    lv_obj_set_style_text_font(label, &lv_font_montserrat_16, 0); // Fuente un poco más grande
    lv_obj_center(label);
}
// ============================================================================
...
void app_main(void)
{
    ...
    if (example_lvgl_lock(-1))
    {
        // LLAMAMOS A NUESTRA INTERFAZ ACÁ ADENTRO DEL LOCK
        ui_init_logger_screen();
