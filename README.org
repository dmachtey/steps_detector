...
#
# Last-Updated: Sun May 31 04:37:58 2026 (-0300)
#+LATEX_CLASS: article
...
(oleaje) mediante técnicas de DSP y ML.
...
* Data Collector
 - Permite recolectar datos de campo, para el entrenamiento de modelo
   de ML, directamente con el teléfono. audio ($8000\text{ Hz}$) y
   acelerometro a ($~60\text{ Hz}$), que luego será subsampleado a
   ($50\text{ Hz}$)
 - logger:
...
Para estructurar el modelo se plantean dos alternativas:
- Opción A (Clasificador Binario): Es la ruta más directa. Cuenta con
  una *Clase 1 (Caminando)*, que agrupa patrones repetitivos de
  impacto en el acelerómetro y audio; y una *Clase 0 (Sin caminar)*,
  que engloba el ruido del viento, agua y perturbaciones ambientales.
- Opción B (Clasificador Multiclase): Agrega mayor robustez frente a
  perturbaciones del entorno. Además de "Caminando" y "Estructura en
  Reposo", se incluye una clase de "Eventos Espurios" para falsos
  positivos como caídas de objetos, impactos bruscos del agua o
...
"sin caminar". Un desbalance (ej. 90% caminata y 10% reposo) generaría
un sesgo en el modelo, haciéndolo tender a clasificar cualquier ruido
como un paso.
...
o /jitter/ entre muestras. Los modelos de Machine Learning requieren
que el intervalo entre datos sea estrictamente constante.
- Se define el primer =timestamp_ms= del archivo CSV como el tiempo
  cero absoluto ($t_0$). El archivo de audio comparte exactamente este
  mismo instante de inicio.
...
entrenamiento.
- Acelerómetro (Z-Score): Se utiliza la Estandarización centrada en
  cero, restando la media aritmética ($\mu$) y dividiendo por la
  desviación estándar ($\sigma$). Esto permite eliminar la gravedad
  terrestre y la inclinación constante de la plataforma.
- Audio (Min-Max Hardware): Se escalan los datos basándose en el
  ...
  por 32768. Esto garantiza valores entre -1 y 1 conservando la
  proporción de la energía real.
...
batería, la limitación principal es la memoria RAM (~520 KB), que
colapsaría al inyectar 24.000 muestras de audio crudo en una red
profunda. La solución es una Arquitectura Híbrida:
- Tratamiento de Audio: La ventana de 3 segundos no se pasa cruda,
  sino que se transforma en un Espectrograma Mel. Esta técnica
  convierte el sonido en una imagen 2D procesada por una Red Neuronal
  Convolucional (CNN).
...
- Fase de Entrenamiento (PC): El procesamiento se realiza localmente
  ...
  red neuronal en la computadora de escritorio.
- Fase de Inferencia (ESP32): Para el entorno de producción, las
  ...
  firmware en C++. El microcontrolador capturará 3 segundos en vivo,
  procesará rápidamente el espectrograma y los 150 puntos de
  aceleración, e inmediatamente borrará la onda cruda de la memoria
  RAM. Los datos transformados se pasarán al modelo híbrido TensorFlow
  Lite para emitir la predicción.
